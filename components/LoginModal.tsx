...
export function LoginModal({ onLoginSuccess, onBackClick }: LoginModalProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-muted/20 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-3xl p-8 shadow-2xl">
          <button
            onClick={onBackClick}
            className="mb-6 text-muted-foreground hover:text-foreground flex items-center gap-2 transition-colors font-medium hover:gap-3"
          >
          ...
          <p className="text-center text-sm text-muted-foreground mt-6">
            ¿No tienes una cuenta? Vuelve y crea una.
          </p>
        </div>
      </div>
    </div>
